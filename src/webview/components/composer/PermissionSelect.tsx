...
export function PermissionGlyph({ value }: { value: string }): JSX.Element {
  ...
  if (value === FULL_ACCESS || value === 'full-access') return <svg viewBox="0 0 16 16" fill="none" aria-hidden><path d={SHIELD} stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round" /><path d="M8.2 4.3v4.5M8.2 10.4v1.2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
...
function displayName(value: string, name?: string): string {
  if (value === FULL_ACCESS || value === 'full-access') return 'Full access'
...
export interface PermissionSelectProps {
  value: PermissionSelectProjection | null
...
export function PermissionSelect({ value, switchingTo, error, zh, locked, onChange }: PermissionSelectProps): JSX.Element | null {
  ...
  const currentName = displayName(value.currentValue, current?.name)
  ...
  return <>
    <div ref={rootRef} className="permission-select" data-composer-tool="permission">
      <button type="button" className="composer-chip permission-trigger" disabled={locked || switchingTo !== null}
        aria-label={zh ? `权限模式，当前：${currentName}` : `Permission mode, current: ${currentName}`}
        aria-haspopup="menu" aria-expanded={open} aria-busy={switchingTo !== null}
        title={current?.description ?? (zh ? `当前权限：${currentName}` : `Current permission: ${currentName}`)}
      ...
      {open && typeof document !== 'undefined' && createPortal(<div ref={menuRef} className="composer-menu permission-menu permission-menu-portal" style={menuPosition} role="menu" aria-label={zh ? '权限模式' : 'Permission mode'}>
        {options.map((option) => <button key={option.value} type="button" role="menuitemradio" aria-checked={option.value === value.currentValue}
          ...
          <span className="composer-menu-item-copy"><span className="composer-menu-item-label">{displayName(option.value, option.name)}</span>{option.description !== undefined && <span className="composer-menu-item-desc">{option.description}</span>}</span>
